...
from tensorflow.python.keras.layers import Layer, Add, Dropout  # pylint: disable=no-name-in-module
from tensorflow.python.keras import activations, initializers  # pylint: disable=no-name-in-module
...
class LayerNormalization(Layer):
    ...
    def build(self, input_shape):
        ...
        self.layer_weights['gain'] = self.add_weight(
            name='gain',
            shape=(dim,),
            initializer='ones',
            trainable=True)
        self.layer_weights['bias'] = self.add_weight(
            name='bias',
            shape=(dim,),
            initializer='zeros',
            trainable=True)
    ...
    def call(self, inputs, **kwargs):
        ...
        result = self.layer_weights['gain'] * normalized_inputs + self.layer_weights['bias']
        return result
...
class TransformerTransition(Layer):
    ...
    def build(self, input_shape):
        ...
        self.transition_weights['weights1'] = self.add_weight(
            name='weights1',
            shape=(d_model, self.size_multiplier * d_model),
            initializer='glorot_uniform',
            trainable=True)
        self.transition_weights['biases1'] = self.add_weight(
            name='biases1',
            shape=(self.size_multiplier * d_model,),
            initializer='zeros',
            trainable=True)
        self.transition_weights['weights2'] = self.add_weight(
            name='weights2',
            shape=(self.size_multiplier * d_model, d_model),
            initializer='glorot_uniform',
            trainable=True)
        self.transition_weights['biases2'] = self.add_weight(
            name='biases2',
            shape=(d_model,),
            initializer='zeros',
            trainable=True)
    ...
    def call(self, inputs, **kwargs):
        ...
        step2 = K.bias_add(
            K.dot(step1, self.transition_weights['weights2']),
...
class TransformerBlock:
    ...
    # cleaner than as a function - separate initialization from actual calls
    """
...
class TransformerACT(Layer):
    ...
    def build(self, input_shape):
        ...
        self.act_weights['halting_biases'] = self.add_weight(
            ...
            trainable=True)
        self.act_weights['time_penalty_t'] = K.constant(self.time_penalty, dtype=K.floatx())
        return super().build(input_shape)
    ...
    def call(self, inputs, **kwargs):
        ...
        self.ponder_cost = (
            self.act_weights['time_penalty_t'] * K.mean(self.remainder + self.active_steps))
