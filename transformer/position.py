...
def positional_signal(hidden_size: int, length: int,
    ...
    inv_timescales = (
        min_timescale *
        K.exp(K.arange(num_timescales, dtype=K.floatx()) * -log_timescale_increment))
...
class AddCoordinateEncoding(AddPositionalEncoding):
    ...
    def call(self, inputs, **kwargs):
        ...
        step_signal = K.expand_dims(self.signal[:, kwargs['step'], :], axis=1)
...
class TransformerCoordinateEmbedding(Layer):
    ...
    def build(self, input_shape):
        ...
        d_model = int(d_model)
        self.embedding_weights['word_position_embeddings'] = self.add_weight(
            shape=(sequence_length, d_model),
            initializer='uniform',
            name='word_position_embeddings',
            trainable=True)
        self.embedding_weights['depth_embeddings'] = self.add_weight(
    ...
    def call(self, inputs, **kwargs):
        ...
        if depth is None:
            raise ValueError("Please, provide current Transformer's step"
                             "using 'step' keyword argument.")
        result = inputs + self.embedding_weights['word_position_embeddings']
        if depth is not None:
            result = result + self.embedding_weights['depth_embeddings'][depth]
        return result
