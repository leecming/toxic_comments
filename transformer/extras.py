...
from tensorflow.python.keras import activations, regularizers  # pylint: disable=no-name-in-module
from tensorflow.python.keras.engine import Layer  # pylint: disable=no-name-in-module
from tensorflow.python.keras.layers import Embedding  # pylint: disable=no-name-in-module
...
class TiedOutputEmbedding(Layer):
    ...
    def build(self, input_shape):
        ...
        _, emb_output_dim = embedding_matrix_shape
        assert len(main_input_shape) == 3
        self.embedding_weights['projection'] = self.add_weight(
            name='kernel',
            shape=(int(main_input_shape[-1]), int(emb_output_dim)),
            initializer='glorot_uniform',
            regularizer=self.projection_regularizer,
            trainable=True)
        if self.add_biases:
            self.embedding_weights['biases'] = self.add_weight(
                name='biases',
                shape=(int(emb_output_dim),),
                initializer='zeros',
                trainable=True)
    ...
    def call(self, inputs, **kwargs):
        ...
        if self.add_biases:
            projected = K.bias_add(projected, self.embedding_weights['biases'],
